...
class Decoder {
	...
	public <T> T[] getSkipSequenceOf(byte type, T[] temp, T inst) throws ASN1DecoderFail {
        ...
        public <T> T[] getSequenceOf(Class<? extends ASNObjArrayable> c, T[] temp, T inst) throws ASN1DecoderFail {
                byte type = Encoder.getByteAnnotationTag(c);
                ArrayList<T> al;
                al = getSequenceOfAL(type, inst);
                if (al == null) return null;
                return al.toArray(temp);
	...
	public double getReal(){
		byte[] value = new byte[contentLength()];
		Encoder.copyBytes(value, 0, data, contentLength(),offset+typeLen()+lenLen());
		return Double.valueOf(new String(value)).doubleValue();
	}
	 ...
	 */
        public boolean fetchAll(InputStream is) throws IOException {
                ...
                while (true) {
                        ...
                        if(DEBUG)System.out.println("Object size="+asrlen+"; Buffer size="+data.length+"; Current size="+length+"; Offset="+offset);
                        ...
                        if ((asrlen < 0) || (length < asrlen)) {
                                if (DEBUG)System.out.println("Object size="+asrlen+"; Current size="+length+"; Offset="+offset);
                                if (length == data.length - offset) return false; // at end of buffer (if not enough for the tag)
                                int inc = is.read(data, length+offset, data.length-(length+offset));
                                if (inc <= 0) {
                                        dumpHexDump();
                                        throw new IOException("End of Stream: " + inc);
                                        //return false; // end of stream
                                }
                                if (DEBUG)System.out.println("Head Byte=" + data[offset] );
                                length += inc;
                ...
                if (DEBUG)System.out.println("Head Byte Ready=" + data[offset] );
                return true;
        }
        /* Length of data including past messages */
/*
	public boolean fetchAll(InputStream is) throws IOException {
		//Decoder dec = new Decoder(sr,0,msglen);
		while (true) {
