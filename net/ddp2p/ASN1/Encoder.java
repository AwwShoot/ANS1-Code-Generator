...
class Encoder {
	...
	*/
	...
	BigInteger contentLength() {
        ...
        public static byte getByteAnnotationTag(Class<? extends ASNObjArrayable> c) {
         ...
         int _tag = a._tag();
         if (_tag < 0 && ! "".equals(a._stag())) 
          _tag =  new BigInteger(a._stag()).intValue();
         
         if (_tag <= 30 && _tag >= 0) {
          return buildASN1byteType(
                                        _class,
                                        _pc,
                                        (byte) a._tag());
         }
         throw new RuntimeException("Annotation type not byte");
        }
	/**
